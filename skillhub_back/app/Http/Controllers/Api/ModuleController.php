...
{
    private const MSG_FORMATION_NOT_FOUND = 'Formation introuvable';
    private const MSG_NOT_OWNER = 'Vous ne pouvez gérer que vos propres formations.';

    public function index(Request $request, int $formationId): JsonResponse
    {
        $formation = Formation::find($formationId);
        if (! $formation) {
            return response()->json(['message' => self::MSG_FORMATION_NOT_FOUND], 404);
        }
    ...
    public function store(StoreModuleRequest $request, int $formationId): JsonResponse
    {
        ...
        if ($formation->id_formateur !== (int) $request->user()->id) {
            return response()->json(['message' => self::MSG_NOT_OWNER], 403);
        }
