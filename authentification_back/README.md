...
Document d’ensemble des trois projets : **[`../README.md`](../README.md)**.
...
│  │  │  ├─ controller/          # Endpoints auth (/api/auth/*)
│  │  │  ├─ service/             # Regles metier auth
│  │  │  ├─ repository/          # Acces BDD
│  │  │  ├─ model/               # Entites JPA
│  │  │  ├─ dto/                 # Requests / Responses
│  │  │  ├─ config/              # Securite, CORS, beans
...
- Ce service centralise les operations compte utilisateur.
- Le frontend l'appelle via `/auth-api` (proxy Vite vers `/api` Spring).
- Laravel l'appelle via `/api/auth/me` pour valider les Bearer tokens.
...
- **Avec le frontend** : inscription/connexion/profil via `/api/auth/*`.
- **Avec Laravel** : validation du Bearer via `GET /api/auth/me` a chaque appel metier protege.
- **Important** : la regle "maximum 5 inscriptions formations" est appliquee dans Laravel (`skillhub_back`), pas dans ce service Auth.
